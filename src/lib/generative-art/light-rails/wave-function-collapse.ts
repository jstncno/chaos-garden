...
function tileTypeFromEdges(start: Edge, end: Edge): TileType {
  if (
    (start == Edge.TOP && end == Edge.RIGHT) ||
    (start == Edge.RIGHT && end == Edge.TOP)
  ) {
    return TileType.TOP_RIGHT;
  } else if (
    (start == Edge.TOP && end == Edge.BOTTOM) ||
    (start == Edge.BOTTOM && end == Edge.TOP)
  ) {
    return TileType.TOP_BOTTOM;
  } else if (
    (start == Edge.TOP && end == Edge.LEFT) ||
    (start == Edge.LEFT && end == Edge.TOP)
  ) {
    return TileType.TOP_LEFT;
  } else if (
    (start == Edge.BOTTOM && end == Edge.RIGHT) ||
    (start == Edge.RIGHT && end == Edge.BOTTOM)
  ) {
    return TileType.BOTTOM_RIGHT;
  } else if (
    (start == Edge.BOTTOM && end == Edge.LEFT) ||
    (start == Edge.LEFT && end == Edge.BOTTOM)
  ) {
    return TileType.BOTTOM_LEFT;
  } else if (
    (start == Edge.LEFT && end == Edge.RIGHT) ||
    (start == Edge.RIGHT && end == Edge.LEFT)
  ) {
    return TileType.LEFT_RIGHT;
  }

  return TileType.UNKNOWN;
}

function tileTypeForEdges(
  p5: P5,
  requiredEdges: Set<Edge>,
  optionalEdges: Set<Edge>
): Set<TileType> {
  const s = new Set<TileType>();
  if (requiredEdges.size == 0 && optionalEdges.size == 0) {
    return s;
  }

  if (requiredEdges.size == 2 && optionalEdges.size < 2) {
    const [start, end] = Array.from(requiredEdges);
    s.add(tileTypeFromEdges(start, end));
    return s;
  }

  if (requiredEdges.size + optionalEdges.size == 4) {
    s.add(TileType.TOPRIGHT_BOTTOMLEFT);
    s.add(TileType.TOPBOTTOM_LEFTRIGHT);
    s.add(TileType.TOPLEFT_BOTTOMRIGHT);
    return s;
  }

  let start: Edge | undefined;
  let end: Edge | undefined;
  for (const edges of [requiredEdges, optionalEdges]) {
    while (edges.size > 0) {
      const edge = p5.random(Array.from(edges));
      edges.delete(edge);
      if (start == undefined) {
        start = edge;
